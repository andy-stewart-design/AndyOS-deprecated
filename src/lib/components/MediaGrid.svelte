<script>
	...
	function setType(width) {
		if (width === '1/2') {
			return 'max-w-xl';
		} else {
			return 'max-w-4xl m-auto';
		}
	}
</script>

<section class="relative grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-24 p-8">
	{#each contents as item, index}
		<div class="{setWidth(item.width)} text-zinc-800 dark:text-zinc-200">
			...
			{:else if item.type === 'text'}
				<div class={setType(item.width)}>
					<!-- <div class="w-full h-px bg-zinc-800 dark:bg-zinc-200 mb-12" /> -->
					<h3 class="text-4xl mb-6" class:hidden={!item.head}>{item.head}</h3>
